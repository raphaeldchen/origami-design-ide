...
// Indexing
  ...
  tmArray<T>& MoveItem(std::size_t inFromIndex, std::size_t inToIndex) {
    ...
    // inFromIndex is 1-based (RemoveItemAt/InsertItemAt below are 1-based), but
    // operator[] is 0-based, so grab element (inFromIndex - 1). The tmArray.h
    // rewrite dropped this "- 1", making MoveItem(size(), ...) read one-past-the-
    // end (garbage/NULL) and insert it in place of the real moved element. That
    // planted a NULL in tmPath::mOwnedVertices (via MakeVertex's insertion sort),
    // which later SIGSEGV'd in tmPath::GetOrMakeVertex on the 5-leaf caterpillar.
    T t = (*this)[inFromIndex - 1];
    RemoveItemAt(inFromIndex);
    InsertItemAt(inToIndex, t);
    return *this;
  }

  // Get item at 1-based index (operator[] is 0-based, so return n - 1; the
  // tmArray.h rewrite dropped this "- 1", a latent one-past-the-end read of
  // the same class as the MoveItem regression above).
