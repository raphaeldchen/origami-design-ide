  ...
  for (size_t i = 0; i < aCreaseList.size(); ++i) {
    ...
    if (CanStartFacetFwd(aCrease)) {
      ...
      do {
        ...
        // Release-build guard: in debug builds the two TMASSERTs at the end of
        // GetNextCreaseAndVertex catch a dead-ended facet walk (no next crease
        // found). With TMASSERT compiled out, a dead end returns
        // nextCrease==thisCrease / nextVertex==null and the next iteration
        // dereferences a null crease -> SIGSEGV. Fail cleanly instead so the
        // caller reports an invalid crease pattern rather than crashing.
        if (!nextVertex || !nextCrease || nextCrease == thisCrease)
          throw std::runtime_error("facet construction failed: the crease "
            "network does not close into facets (the base is not a complete "
            "uniaxial optimum / the molecule is degenerate)");
        ...
        tooMany++;
        // Release-build guard: the original TMASSERT(tooMany<100) is compiled
        // out under NDEBUG, so a facet walk that loops without ever returning to
        // firstVertex hangs forever. Fail cleanly instead.
        if (tooMany >= 100)
          throw std::runtime_error("facet construction failed: the forward "
            "facet walk did not close (looping crease network)");
    ...
    if (CanStartFacetBkd(aCrease)) {
      ...
      do {
        GetNextCreaseAndVertex(thisCrease, thisVertex, nextCrease, nextVertex);
        // Release-build guard against a dead-ended facet walk (see fwd loop).
        if (!nextVertex || !nextCrease || nextCrease == thisCrease)
          throw std::runtime_error("facet construction failed: the crease "
            "network does not close into facets (the base is not a complete "
            "uniaxial optimum / the molecule is degenerate)");
        ...
        thisVertex = nextVertex;
        tooMany++;
        if (tooMany >= 100)  // see forward-walk guard above
          throw std::runtime_error("facet construction failed: the backward "
            "facet walk did not close (looping crease network)");
      } while (nextVertex != firstVertex);
