...
void tmCrease::CalcBend()
{
  ...
  // Release-build guard: GetAxialOrGussetCreases ends in a TMFAIL (compiled out
  // here) when the bottom vertex does not have its two flanking axial/gusset
  // creases, leaving crease1/crease3 null. Dereferencing them then segfaults.
  // Bail out cleanly so the crease keeps its current (hinge) kind and the tree
  // reports an incomplete crease pattern instead of crashing the engine.
  if (!crease1 || !crease3) return;
