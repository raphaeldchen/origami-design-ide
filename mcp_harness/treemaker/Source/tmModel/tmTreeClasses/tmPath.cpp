...
tmVertex* tmPath::MakeVertex(const tmPoint& p, tmNode* aTreeNode)
{
  ...
  // Create the vertex. The tmVertex constructor ALREADY registers it with both
  // the tree (mTree->mVertices) and this owner path (mVertexOwner->mOwnedVertices,
  // see tmVertex.cpp:96), so it must NOT be pushed again. A prior refactor wrapped
  // the result in a unique_ptr and did an explicit mOwnedVertices.push_back() on
  // top of that self-registration, double-listing every path-owned vertex. The
  // duplicate then propagated into GetRidgelineVertices() and ConnectSelfVertices(),
  // which creased consecutive (identical) vertices together -> zero-length
  // self-loop creases (a vertex creased to itself). Those degenerate creases gave
  // GetAxialOrGussetCreases() a flank looping back to the bottom vertex, so CalcBend
  // saw depth2==depth3 and mislabeled hinges, inverting facet two-coloring parity
  // and leaving structural creases FLAT -> the crease pattern failed flat-
  // foldability. tmPoly.cpp:639 asserts (debug-only) that ridgeline vertices are
  // never double-listed; this restores that invariant.
  tmVertex* rawVertex = new tmVertex(mTree, this, p, elevation,
                                     static_cast<bool>(mIsBorderPath), aTreeNode);
  ...
  for (size_t i = 0; i < mOwnedVertices.size() - 1; ++i) {
    ...
    for (auto* theCrease : mOwnedCreases) {
      ...
      if (x > 0 && x < 1) {
        ...
        // Split the crease in two. Use the FULL tmCrease constructor: it
        // registers each new crease with its owner path AND, crucially, pushes
        // the crease onto BOTH endpoint vertices' mCreases lists (tmCrease.cpp
        // lines 88-89). The facet-construction walk (tmFacetOwner::
        // GetNextCreaseAndVertex) traverses thisVertex->mCreases, so omitting
        // that back-link leaves split vertices with an incomplete incidence
        // list -> the walk dead-ends -> null deref -> SIGSEGV. A prior refactor
        // built these with the bare tmCrease(mTree) ctor and wired only
        // crease->vertex, never vertex->crease; that is what broke the molecule
        // builder. Both new creases are created before the old one is deleted.
        new tmCrease(mTree, this, frontVertex, rawVertex, theCrease->mKind);
        new tmCrease(mTree, this, rawVertex, backVertex, theCrease->mKind);
        delete theCrease;
        break;
      }
    }
...
void tmPath::ConnectSelfVertices(tmCrease::Kind aKind)
{
  ...
  for (size_t i = 0; i < mOwnedVertices.size(); ++i) {
    ...
    // GetOrMakeCrease returns a crease OWNED by the tree's part hierarchy (it
    // registers the new crease with this owner and both endpoint vertices, or
    // returns a pre-existing one). The caller must NOT take ownership. A prior
    // refactor wrapped this non-owning pointer in a std::unique_ptr, so the
    // crease was deleted at the end of each iteration (and ~tmCrease then
    // stripped it from both vertices' mCreases lists) -- ConnectSelfVertices
    // built the axial/gusset/hinge creases and immediately destroyed them, so
    // the crease network never closed into facets. Discard the return value.
...
bool tmPath::TestIsFeasible(const tmFloat& actLen, const tmFloat& minLen) {
  // Path is feasible if actual length is >= minimum length, to within the SAME
  // distance tolerance used for activity (DistTol()=1e-4). The original used
  // DistTol(); a refactor changed it to TMFLOAT_TOL (1e-10), which is far
  // tighter than activity's IsTiny()/DistTol(). That mismatch makes a binding
  // path whose length lands ~1e-7 below minimum (ordinary optimizer slack)
  // ACTIVE yet INFEASIBLE -- and an infeasible path strips polygon-node status
  // from both its endpoints (tmTree::CalcPolygonNetwork), collapsing the whole
  // polygon partition to zero polys. Feasibility must be at least as lenient as
  // activity.
...
/*****
Calculate the minimum and actual lengths of this path in both tree and paper
space, then set the feasibility/activity flags from them.

The MINIMUM length is intrinsic to the tree: the sum of the (strained) edge
lengths along the path. The ACTUAL length is a property of the current PACKING:
the straight-line (paper) distance between the path's two end nodes, wherever
the optimizer has placed them. A leaf path is ACTIVE when those two are equal
(the separation constraint is binding) and FEASIBLE when the actual distance is
at least the minimum. Both comparisons are made in paper space.

NOTE: deriving the actual length from the node coordinates is essential -- the
whole point of the scale optimizer is to move the nodes, and the active-path set
(hence the polygon partition the Universal Molecule is built on) depends on where
they ended up. A prior refactor computed mActPaperLength as mActTreeLength*scale,
which ignores the packing entirely and makes every unstrained path trivially
"active", so the polygon network never validates. See tmTree::CalcPolygonValidity.
*****/
void tmPath::TreePathCalcLengths() {
  // Only call this for tree paths.
  TMASSERT(IsTreePath());

  // Minimum length: sum of the (strained) edge lengths along the path.
  mMinTreeLength = 0.0;
  for (size_t i = 0; i < mEdges.size(); ++i)
    mMinTreeLength += mEdges[i]->GetStrainedLength();
  mMinPaperLength = mMinTreeLength * mTree->GetScale();

  // Actual length: Euclidean distance between the path's end nodes on the
  // paper. Only meaningful for leaf paths; non-leaf paths get an innocuous 0.
  if (mIsLeafPath && mNodes.size() > 0)
    mActPaperLength = Mag(mNodes.front()->mLoc - mNodes.back()->mLoc);
  else
    mActPaperLength = 0.0;
  const tmFloat scale = mTree->GetScale();
  mActTreeLength = (scale != 0.0) ? (mActPaperLength / scale) : 0.0;

  // Feasibility/activity are paper-space comparisons, leaf paths only.
  if (mIsLeafPath) {
