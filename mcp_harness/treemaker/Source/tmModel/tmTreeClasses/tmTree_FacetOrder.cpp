...
// END DEBUGGING
  ...
  // Release-build guard: if no facet qualifies as a source (every facet has a
  // tail), the facet-ordering graph still contains a cycle that BreakOneLink
  // did not resolve, so it cannot be linearized. The debug build would have
  // tripped an assertion upstream; here sourceFacet is null and CalcOrder would
  // dereference it. We must NOT throw -- CalcFacetOrder runs inside
  // tmTreeCleaner::~tmTreeCleaner (a destructor), and throwing there calls
  // std::terminate. Instead return without ordering; the facets keep mOrder==-1
  // and the tree's facet data stays invalid, so HasFullCP() is false and the
  // caller reports an incomplete crease pattern rather than crashing.
  if (!sourceFacet) {
    mIsFacetDataValid = false;
    // CleanupAfterEdit does `CalcFacetOrder(); if (!mIsLocalRootConnectable)
    // return;` -- clearing this flag makes it skip the rest of the pipeline
    // (CalcFacetColor / CalcFold) which would otherwise dereference the
    // never-set facet order and crash. HasFullCP() is false either way.
    mIsLocalRootConnectable = false;
    return;
  }
  size_t nextOrder = 0;
